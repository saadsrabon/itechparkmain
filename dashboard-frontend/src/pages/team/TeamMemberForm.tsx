...
export default function TeamMemberForm({ initial, onSuccess, onCancel }: Props) {
  const [section, setSection] = useState(initial?.section ?? 'Core Team')
  ...
  useEffect(() => {
    setSection(initial?.section ?? 'Core Team')
  ...
  const submit = async () => {
    try {
      ...
      if (initial?._id) {
        await teamApi.update(initial._id, fd)
      } else {
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      <div>
        ...
        <textarea value={content} onChange={(e) => setContent(e.target.value)} style={{ ...input, height: 100 }} />
      </div>
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        {imagePreview && <img src={imagePreview} alt="preview" style={{ width: 64, height: 64, objectFit: 'cover', borderRadius: 8, border: '1px solid #334155' }} />}
